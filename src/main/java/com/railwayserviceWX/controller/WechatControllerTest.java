 ...
 */
@Controller
@RequestMapping(value = "/wechatServiceTest", produces = {"application/json;charset=UTF-8"})
@Api(value = "微信通用接口测试控制器", description = "微信通用接口测试控制器")
public class WechatControllerTest {
	
	public static StringBuffer currentOpenid;
	
	public static Boolean isTest = false;
	
    private final Logger logger = LoggerFactory.getLogger(WechatControllerTest.class);
    ...
    public void servletGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        logger.info("TOKEN:"+WeixinConfig.TOKEN);
        list.add(timestamp);
        list.add(nonce);
        // 排序
    ...
    public void servletPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        session.addOnHandleMessageListener(new HandleMessageAdapter() {
            ...
            public void onEventMsg(final EventMsg msg) {
                ...
                //测试账号入口，通过点击链接进入
                if ("index".equals(eventKey)) {
                	logger.info("点击index按键，openid="+openid);
                	...
                	currentOpenid = null;
                	currentOpenid = new StringBuffer(openid);
                	reMsg.setContent("<a href='"+WeixinConfig.HOST+"/wechat/index.html'>点击进入《站内点餐》</a>");
					session.callback(reMsg);//回传消息
                }
                
                if ("ticket".equals(eventKey)) {
                	logger.info("点击ticket按键，openid="+openid);
                	User user = userService.getUserByOpenid(openid);
                	...
                	currentOpenid = null;
                	currentOpenid = new StringBuffer(openid);
                	reMsg.setContent("<a href='"+WeixinConfig.HOST+"/wechat/trainTickets/index.html'>点击进入《抢火车票》</a>");
					session.callback(reMsg);//回传消息
                }
            }
            ...
            public void onTextMsg(TextMsg msg) {
                ...
                reMsg.setContent("<a href='http://119.23.134.10/RailwayService/wechat/trainTickets/index.html'>点击进入《抢火车票》</a>");
                //回传消息
